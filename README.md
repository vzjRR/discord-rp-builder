...
npm run test:names   # يفحص رسم الأسماء (عربي/مختلط/رياضي/عريض) بدون أي اتصال بديسكورد
npm run dry-run      # محاكاة دخول عضو كاملة: يفحص القنوات والرول ويطبع الرسالة
```

`test:names` هو حارس الانحدار للأسماء العربية — يتأكد إن الحروف تتوصل فعلًا ويحفظ صور
معاينة بـ `test-names-output/`. شغّله بعد أي تعديل على الخطوط أو على `composeWelcomeImage.js`.
...
**الخطوط:** `assets/fonts/` فيها Barlow + Noto (Math / Arabic / Sans) وكلها مرفوعة مع الكود.
