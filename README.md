...
مشروع مستقل بمجلد `points-bot/` — له `package.json` خاص فيه وعملية Node منفصلة
(خدمة systemd خاصة بها)، لكنه يستخدم **نفس** تطبيق ديسكورد وتوكن Enclave
المستخدم بـ `build.js`/`logs-bot`/`welcome-bot` — لا تطبيق منفصل، Discord
يسمح لتوكن واحد يشغّل عدة اتصالات gateway بالتوازي. يراقب قناة واحدة محددة
ويحتسب نقطة تلقائية لكل رسالة فيها صورة — **بلا أي أمر ديسكورد إطلاقًا**، لا
slash ولا نصي. كل القراءة والتعديل اليدوي تتم من `admin-panel` (صفحة
"ترتيب نقاط الدعم"، صلاحيتا `points.view` و`points.manage` يمنحهما المالك
من صفحة "إدارة الحسابات").
